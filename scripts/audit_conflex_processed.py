...
INPUT_PATH = Path("data/processed/CONFLEX.csv")
OUTPUT_PATH = Path("reports/tables/conflex_parse_audit.csv")


def normalize_default_voltage(mark: str) -> str:
    if extract_rated_voltage_v(mark) is not None:
        return mark
...
def main() -> None:
    ...
    parser = ConflexParser()
    generator = GENERATORS["conflex"]
    rows = []
    errors = []
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            normalized_source = normalize_default_voltage(mark)
