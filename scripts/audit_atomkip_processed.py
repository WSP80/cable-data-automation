...
from src.parsers.atomkip.rules import extract_sheath_color
from src.parsers.atomkip.parser import AtomkipParser


INPUT_PATH = Path("data/processed/ATOMKIP-KU.csv")
OUTPUT_PATH = Path("reports/tables/atomkip_parse_audit.csv")


def normalize_generated_text_suffix(mark: str) -> str:
    normalized_suffix = extract_sheath_color(mark)

    if normalized_suffix is None:
        return mark

    voltage_match = pd.Series([mark]).str.extract(
        r"(?P<voltage>\d{3,4}\s*[ВV])",
        expand=True,
    )["voltage"].iloc[0]

    if pd.isna(voltage_match):
        return mark

    voltage_end = mark.find(voltage_match) + len(voltage_match)

    return f"{mark[:voltage_end].strip()} ({normalized_suffix})"


def main() -> None:
    df = pd.read_csv(INPUT_PATH, sep=";", encoding="utf-8")
    parser = AtomkipParser()
    generator = GENERATORS["atomkip"]
    rows = []
    errors = []
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            normalized_source = normalize_generated_text_suffix(mark)
